<template>
  <v-menu v-if="$auth.user" offset-y rounded="0" content-class="elevation-2" class="navbar-v-menu">
    <template v-slot:activator="{ on, attrs }"  >
      <a class="nav-link-item text-uppercase link--text" :class="{'ml-7':marginLeft}" v-bind="attrs" v-on="on" style="display: flex;">
    ...
    <v-list >
      <v-list-item>
        <NuxtLink class="nav-link-item text-uppercase link--text" to="/profile/">Profile</NuxtLink>
      </v-list-item>
      <v-list-item v-if="$auth.user.role === 'admin'">
        <NuxtLink class="nav-link-item text-uppercase link--text" to="/admin/">Admin</NuxtLink>
      </v-list-item>
      <v-list-item>
        <v-btn
          @click="logout"
